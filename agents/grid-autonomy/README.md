...
| Bot stopped itself when price left the grid channel | **Fixed (2026-09-06):** deploys and edits now set `stopOnOutOfGrid: false` — WunderTrading used to stop the bot and close all its open lines at market when price exited the channel, realizing losses. Out-of-channel bots are owned by the daemon now: losing ones are re-centered (`recenter`/`adjust` journal, verified live to leave open positions untouched), profitable ones follow the normal re-analysis/swap flow. |
| How does a bot ever exit / recycle its slot? | The **daemon-side profit exit** (`profit-exit` journal): when a bot's cumulative total PnL — realized round-trips + mark PnL of open lines — reaches `grid_defaults.take_profit_pct` × its slot budget (default 10%) AND every open line is ≥ 0, it is stopped at profit and the slot recycles to the best candidate. WunderTrading's native takeProfit/stopLoss/trailing fields are accepted by the API but **not enforced server-side for grid bots** — the daemon owns the exit. Losing bots are never closed (see the loss-veto rule above); they are held and re-centered until they trade back. |
...
| Rotation won’t happen | Requires a stagnant incumbent **and** Δscore ≥ 5 **and** expired per-token cooldown. Check `/status` journal for `rotation-veto` / `rotation-skip`. **Held under water:** an incumbent whose mark PnL is negative is never rotated/swapped (`loss-veto` journal) — its open positions must not be closed at a loss; it keeps running until break-even or better. |
| Optimizer closed a position at a loss | Should never happen (hard rule, enforced in `execute_rotation` + `is_idle`): any stop that would realize a mark loss is vetoed. If you saw it before 2026-09-05 20:15 UTC, that was the pre-fix optimizer (`stop_and_close_all` on every swap). |
...
374 tests as of 2026-09-06, all offline (network/browser calls are mocked
