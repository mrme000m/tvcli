...
Env (all required; absent → exit 0 with a warning for the entrypoint):
...
def render(account_id: str, api_key: str, config_dir: Path) -> list:
    ...
    if json.dumps(auth, sort_keys=True) != before:
...
def main() -> int:
    ...
    # presence-only reporting — the key value never leaves the process
    print("prime_agent_config: config_dir=%s changed=%s" % (config_dir, changed))
    return 0
