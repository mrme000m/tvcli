...
1. **Agent Skills** (agentskills.io standard) in `.agents/skills/` — loaded
   ...
   | `grid-autonomy` | Operate/run/troubleshoot the autonomous grid-trading daemon in `agents/grid-autonomy/` — screen → deliberate → guard → deploy → watch → rotate loop on WunderTrading paper profiles, 8 fail-closed guardrails, plan-capacity-aware deploys (auto-observed subscription limits: HYPERLIQUID_SWAP premium/200 via WT's 0.035% builder fee, 1 active grid bot on other exchanges' free tier; the 5 demo/paper-bot cap is learned from the create-400 and vetoes new deploys at the cap), best-token screening over a broad universe (≥ `$2M` 24h volume, top-100/venue) with tvcli `/hunt` numeric fitness + expected-value grid-fill ranking, dynamic slot opening for Hyperliquid by default (slots open while a token clears `screen.open_slot_min_score` AND spare capital covers the worst case — capital is the ceiling, `slots_hard_max`/`min_slot_usd` the guards; budgets re-reconciled to config at startup), HTTP ctl on :8799, decisions.jsonl + run cards, and the mission console UI + JSON API on :8798 (`console/` — observe/configure/dev-control: channel-ladder fleet cards, decision ledger, reliability tiers, whitelisted config.yaml editor, confirm-gated lifecycle ops). Operate everything via the `./dev` script, incl. `dev restart daemon|console` component restarts (console code changes need `dev restart console`). |
