...
1. **Agent Skills** (agentskills.io standard) in `.agents/skills/` — loaded
   ...
   | `wundertrading` | WunderTrading execution layer — classify a token's market regime from public OHLCV, map it to a classic/DCA strategy config (TP ladders, stops, trailing, DCA sizing math), deploy/manage via wun MCP tools or HMAC-signed REST `/open_api` (both now also via `wt_httpx.py` httpx without browser, `browser` parity via `wt_browser.py` CDP fetch-in-page), prove reliability from exported strategy history, and read plan limits (dashboard `account-limits` vs the enforced per-exchange tier caps in the `grid_bots/upsert` init data — free plan: 1 active grid bot off-Hyperliquid, HYPERLIQUID_SWAP premium/200 via 0.035% builder fee). MCP server `wundertrading` is configured in zcode + PrimeAgent. |
   ...
   | `grid-autonomy` | Operate/run/troubleshoot the autonomous grid-trading daemon in `agents/grid-autonomy/` — screen → deliberate → guard → deploy → watch → optimize → rotate loop on WunderTrading paper profiles, 8 fail-closed guardrails, plan-capacity-aware deploys (auto-observed subscription limits: HYPERLIQUID_SWAP premium/200 via WT's 0.035% builder fee, 1 active grid bot on other exchanges' free tier; the 5 demo/paper-bot cap is learned from the create-400 and vetoes new deploys at the cap, relearned upward on the health cycle when live bots exceed it), best-token screening over a broad universe (≥ `$2M` 24h volume, top-100/venue) with tvcli `/hunt` numeric fitness + expected-value grid-fill ranking, a fast 2–5 min slot optimizer (`optimizer.py`: token-relative idle detection, challenger re-hunt on live candles + tvcli 15m structure, Mistral-pinned LLM arbiter, swaps via execute_rotation with churn bounds — GET /optimizer, POST /optimize on :8799), a slow 15 min + on-entry position optimizer (`position_optimizer.py`: fresh-candle grid revaluation, TP/SL/trailing exit scoring, advisory recommendations persisted to the PocketBase `recommendations` collection when expected profit improvement ≥ threshold — GET /api/recommendations on :8798; `apply: false` never auto-edits WT — see `docs/position_optimizer.md`), dynamic slot opening for Hyperliquid by default (slots open while a token clears `screen.open_slot_min_score` AND spare capital covers the worst case — capital is the ceiling, `slots_hard_max`/`min_slot_usd` the guards; budgets re-reconciled to config at startup), HTTP ctl on :8799, decisions.jsonl + run cards, and the mission console UI + JSON API on :8798 (`console/` — observe/configure/dev-control: channel-ladder fleet cards, decision ledger, reliability tiers, whitelisted config.yaml editor, confirm-gated lifecycle ops). Operate everything via the `./dev` script, incl. `dev restart daemon|console` component restarts (console code changes need `dev restart console`). |
   | `grid-autonomy` (deployment note) | The Mac's local daemon and the az00 VPS deployment run on **two SEPARATE WunderTrading accounts** (local CloakBrowser session on CDP :9222 vs the vault item `wundertrading`, folder `grid-autonomy`, which `wt-login.mjs` uses inside the container; dev-side access to the vault account: `browser-debug/wt-exchanges-live.py` on CDP :9223 — never :9222). Fleets/profiles/bots do not collide; `dev reset-wt` is account-scoped. Deployment defaults to `GRID_MODE=live-paper` (image env, `vps-run.sh` first-deploy fallback, workflow dispatch default); the console header + fleet-summary "WT account" row show which account an instance trades on (`WT_ACCOUNT_LABEL` overrides). |
   ...
   | `cloudflare-one` | Cloudflare One Zero Trust guidance (vendored from [cloudflare/skills](https://github.com/cloudflare/skills), Apache-2.0): Access, Gateway, WARP, Tunnel/cloudflared connectors, private networking, DLP, CASB. Use when designing/troubleshooting the grid-autonomy tunnel + public `*.00m.indevs.in` hosts; pairs with the operational `cf` skill (vault-backed tunnel/DNS commands). |
