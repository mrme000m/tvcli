...
1. **Agent Skills** (agentskills.io standard) in `.agents/skills/` — loaded
   ...
   | `grid-autonomy` | Operate/run/troubleshoot the autonomous grid-trading daemon in `agents/grid-autonomy/` — screen → deliberate → guard → deploy → watch → rotate loop on WunderTrading paper profiles, 8 fail-closed guardrails, HTTP ctl on :8799, decisions.jsonl + run cards. |
