// Package extract — schema-guided extraction layer.
// This file adds ExtractWithSchema, which uses PineSchema ground-truth
// types instead of statistical inference when available.
package extract
...
func ExtractWithSchema(pineID, symbol, timeframe string, parsed *dynparse.ParseResult, graphic map[string]map[string]any, strategyReport map[string]any) *Signals {
	sch := parsed.Schema
...
func calcStatsFromTyped(bars []dynparse.TypedBar, fields []string) map[string]stats {
...
func findValue(bar dynparse.TypedBar, name string) (float64, bool) {
...
func detectEventsFromTyped(bars []dynparse.TypedBar, fields []string, classes map[string]PlotClass) []Event {
	var events []Event
	// Sort bars oldest-first for event detection
	sorted := make([]dynparse.TypedBar, len(bars))
	copy(sorted, bars)
...
func extractLevelsFromTyped(bars []dynparse.TypedBar, fields []string, classes map[string]PlotClass, dominant float64) []Level {
	var levels []Level
	for _, f := range fields {
