...
func TestAllSkills_NoDataSmoke(t *testing.T) {
	for _, s := range skill.All() {
		...
		t.Run(s.Name, func(t *testing.T) {
			assertNoData(t, s.Name, func() skill.SkillResult {
				return s.ParseOutput(nil, nil, "5m", "OANDA:XAUUSD", map[string]string{})
			})
			if s.ParseWithSchema != nil {
				assertNoData(t, s.Name+"[schema]", func() skill.SkillResult {
...
func assertNoData(t *testing.T, name string, run func() skill.SkillResult) {
	t.Helper()
