...
export function startWebSocketCollection(
  ...
  registry.registerTyped(typed, 'Network.webSocketCreated', (params) => {
    ...
    // Also push to the store array so open/live connections are visible via
    // peek/HAR during an active session — not only after the socket closes
    // or the session stops. Long-lived sockets (e.g. TradingView's chart data
    // stream) never close, so without this they'd be invisible while live.
  ...
  registry.registerTyped(typed, 'Network.webSocketClosed', (params) => {
    ...
    // The connection is already in the store array (added on create); just
    // drop the lookup-map entry.
  ...
  return () => {
    // All created connections were already pushed to the store array on
    // `webSocketCreated`, so do not flush the remaining map here — that would
    // duplicate them.
